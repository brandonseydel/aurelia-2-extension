...
const tokenTypes = [node_1.SemanticTokenTypes.variable, node_1.SemanticTokenTypes.property, node_1.SemanticTokenTypes.function, node_1.SemanticTokenTypes.method, node_1.SemanticTokenTypes.keyword, node_1.SemanticTokenTypes.class, node_1.SemanticTokenTypes.type, node_1.SemanticTokenTypes.parameter];
const tokenModifiers = [node_1.SemanticTokenModifiers.declaration, node_1.SemanticTokenModifiers.definition, node_1.SemanticTokenModifiers.readonly];
...
function log(level, message, ...optionalParams) {
    ...
    if (LOG_LEVEL_ORDER[level] >= LOG_LEVEL_ORDER[currentLevel]) {
        ...
        if (optionalParams.length > 0) {
            try {
                const paramsString = optionalParams.map(p => typeof p === 'object' ? JSON.stringify(p) : String(p)).join(' ');
        ...
        switch (level) {
            case 'error':
                connection.console.error(logMessage);
                break;
            case 'warn':
                connection.console.warn(logMessage);
                break;
            case 'info':
                connection.console.info(logMessage);
                break;
            case 'log':
            case 'debug':
...
function isAureliaAttribute(attrName) {
...
function extractExpressionsFromHtml(htmlContent) {
    const expressions = [];
    ...
    const traverse = (node) => {
        ...
        if (typeof node.childNodes === 'object' && node.childNodes !== null) {
...
function calculateLocationFromOffset(content, targetOffset) {
    let line = 1;
    let col = 1;
    ...
    if (targetOffset > content.length)
...
function updateVirtualFile(htmlUri, htmlContent) {
    ...
    const virtualFileUriString = vscode_uri_1.URI.parse(htmlUri + ".virtual.ts").toString();
    const virtualFsPath = vscode_uri_1.URI.parse(virtualFileUriString).fsPath;
    ...
    const htmlUriString = vscode_uri_1.URI.parse(htmlUri).toString();
...
function createLanguageServiceInstance(workspaceRoot) {
    let compilerOptions = {
    ...
    let projectFiles = []; // Track files included by tsconfig
    if (configFileName) {
        ...
        if (configFile.error) {
            ...
            const parsedConfig = ts.parseJsonConfigFileContent(configFile.config, ts.sys, projectDir, // Context directory
    ...
    const host = {
        getScriptFileNames: () => {
            ...
            projectFiles.forEach(filePath => fileUris.add(vscode_uri_1.URI.file(filePath).toString()));
            // +++++++++++++++++++++++++++++++++++++++++++
            const uniqueFsPaths = Array.from(fileUris).map(uri => vscode_uri_1.URI.parse(uri).fsPath);
        ...
        getScriptVersion: (fileName) => {
            const fileUri = vscode_uri_1.URI.file(fileName).toString(); // Convert fsPath back to URI string
        ...
        getScriptSnapshot: (fileName) => {
            const fileUri = vscode_uri_1.URI.file(fileName).toString();
    ...
    for (const sourceFile of sourceFiles) {
        ...
        ts.forEachChild(sourceFile, node => {
            if (ts.isClassDeclaration(node) && node.name) {
                ...
                let foundElementType = undefined;
                if (decorators && decorators.length > 0) {
                    ...
                    for (const decorator of decorators) {
                        ...
                        const fileUri = vscode_uri_1.URI.file(sourceFile.fileName).toString();
...
function updateComponentInfoForFile(fileUri) {
    if (!languageService || !workspaceRoot)
        return false;
    const program = languageService.getProgram();
    if (!program)
        return false;
    const sourceFile = program.getSourceFile(vscode_uri_1.URI.parse(fileUri).fsPath);
    ...
    const filePath = vscode_uri_1.URI.parse(fileUri).fsPath;
    // Check existing entries for this file URI to see if we need to remove old ones first
    let existingComponentName = undefined;
                        ...
                        if (elementName) {
                            ...
                            if (existingComponentName !== elementName || !aureliaProjectComponents.has(elementName)) {
                                if (existingComponentName)
                                    aureliaProjectComponents.delete(existingComponentName); // Remove old if name changed
...
documents.onDidChangeContent((change) => {
    ...
    const htmlUriString = vscode_uri_1.URI.parse(uri).toString(); // Use normalized URI string
...
connection.onCompletion((params) => {
    ...
    if (!document || !htmlUriString.endsWith('.html'))
        return undefined;
    ...
    if (!activeMapping) {
        ...
        try {
            ...
            let currentAttributeName = undefined; // Store partially typed attribute
            function findContext(node, parent) {
                ...
                if (loc.startOffset <= relativeOffset && relativeOffset <= loc.endOffset) {
                    ...
                    if ('childNodes' in node && node.childNodes.length > 0) {
                        node.childNodes.forEach(child => findContext(child, node));
                    }
                }
            }
            ...
            // +++ MOVE Dot trigger check BEFORE the fallback check +++
                ...
                const textBeforeOffset = document.getText(node_1.Range.create(node_1.Position.create(0, 0), params.position));
                ...
                if (tagNameFromRegex) {
                    ...
                    if (componentInfo?.type === 'element' && componentInfo.bindables && Array.isArray(componentInfo.bindables)) {
                        const textEndingBeforeDot = document.getText(node_1.Range.create(node_1.Position.create(0, 0), document.positionAt(offset - 1)));
                        ...
                        if (wordBeforeDot && componentInfo.bindables.includes(wordBeforeDot)) {
                            ...
                            htmlCompletions.forEach(item => { if (!distinctMap.has(item.label)) {
            ...
            if (provideElementCompletions) {
                ...
                aureliaProjectComponents.forEach((info) => {
                    if (info.type === 'element') {
                        ...
                        // ++++++++++++++++++++++++++++++++++++++
                        htmlCompletions.push({ label: info.name, kind: node_1.CompletionItemKind.Class, detail: `Au Element (${path.basename(vscode_uri_1.URI.parse(info.uri).fsPath)})` });
                    }
                });
            }
                ...
                aureliaProjectComponents.forEach((info) => {
                    if (info.type === 'attribute') {
                        htmlCompletions.push({
                            ...
                            detail: `Au Attribute (${path.basename(vscode_uri_1.URI.parse(info.uri).fsPath)})`,
                        });
                ...
                if (currentTagName) {
                    ...
                    if (componentInfo?.type === 'element' && componentInfo.bindables && Array.isArray(componentInfo.bindables)) {
                        ...
                        componentInfo.bindables.forEach(bindableName => {
                            ...
                            htmlCompletions.push({
                                label: bindableName,
                                kind: node_1.CompletionItemKind.Property, // Bindable property
                            ...
                            AURELIA_BINDING_SUFFIXES.forEach(suffix => {
                                ...
                                if (suffix !== '.ref') {
                                    htmlCompletions.push({
                                        ...
                                        kind: node_1.CompletionItemKind.Event, // Or Property/Function?
            ...
            if (triggerChar === '<' || charBeforeCursor === '<') {
                ...
                if (tagMatch) {
                    ...
                    aureliaProjectComponents.forEach((info) => {
                        if (info.type === 'attribute') {
                            htmlCompletions.push({ label: info.name, kind: node_1.CompletionItemKind.Property, insertText: `${info.name}="$1"`, insertTextFormat: node_1.InsertTextFormat.Snippet, detail: `Au Attribute (${path.basename(vscode_uri_1.URI.parse(info.uri).fsPath)})` });
                            AURELIA_BINDING_SUFFIXES.forEach(suffix => { if (suffix !== '.ref') {
                                htmlCompletions.push({ label: `${info.name}${suffix}`, kind: node_1.CompletionItemKind.Event, insertText: `${info.name}${suffix}="\${1:expression}"`, insertTextFormat: node_1.InsertTextFormat.Snippet, detail: `Bind ${suffix} on ${info.name}` });
                            } });
                        }
                    });
                    AURELIA_TEMPLATE_CONTROLLERS.forEach(controller => { htmlCompletions.push({ label: controller, kind: node_1.CompletionItemKind.Struct, insertText: `${controller}="\${1:expression}"`, insertTextFormat: node_1.InsertTextFormat.Snippet, detail: `Aurelia Template Controller` }); });
                }
            }
    ...
    if (docInfo && activeMapping) {
        ...
        let result = []; // Placeholder for expression results variable
        ...
        const virtualFsPath = vscode_uri_1.URI.parse(docInfo.virtualUri).fsPath;
        log('debug', `[onCompletion] Requesting completions for virtual file path: ${virtualFsPath} at offset ${virtualCompletionOffset}`);
        let completions;
        try {
            log('debug', `[onCompletion] Requesting completions for virtual file path: ${virtualFsPath} at offset ${virtualCompletionOffset}`);
            completions = languageService.getCompletionsAtPosition(virtualFsPath, virtualCompletionOffset, {
        ...
        // --- Process completions --- 
        let expressionCompletions = []; // <<< RENAME variable
        const desiredKinds = [
        ...
        expressionCompletions = completions.entries
            .filter(entry => {
            if (entry.name.startsWith('___expr_') || entry.name.startsWith('_this') || entry.name.startsWith('__filename') || entry.name.startsWith('__dirname'))
                return false;
            ...
            if (isViewModelMember)
                sortPriority = '0'; // Highest for VM members
            // else if (entry.kind === ts.ScriptElementKind.keyword) sortPriority = '1'; // <<< REMOVE OLD KEYWORD PRIORITY
            else
                sortPriority = '5'; // Standard priority for non-VM members (methods, properties)
        ...
        const originalHtmlText = document.getText(node_1.Range.create(document.positionAt(activeMapping.htmlExpressionLocation.startOffset), document.positionAt(activeMapping.htmlExpressionLocation.endOffset)));
        ...
        if (textBeforeCursor.trim() === '' && !expressionCompletions.some(item => currentMemberNamesFallback.includes(item.label))) {
            ...
            currentMemberNamesFallback.forEach((memberName) => {
...
connection.onDefinition(async (params) => {
    ...
    if (!document || !htmlUri.endsWith('.html'))
        return undefined;
    ...
    const originSelectionRange = node_1.Range.create(document.positionAt(originHtmlStartOffset), document.positionAt(originHtmlEndOffset));
    ...
    for (const def of definitionInfo.definitions) {
        ...
        if (def.fileName === docInfo.virtualUri)
            continue;
        const targetUri = vscode_uri_1.URI.file(def.fileName).toString();
        ...
        const targetRange = node_1.Range.create(targetStartPos.line, targetStartPos.character, targetEndPos.line, targetEndPos.character // Correct end character
        );
        // Selection range usually just the start of the definition for navigation
        const targetSelectionRange = node_1.Range.create(targetStartPos.line, targetStartPos.character, targetStartPos.line, targetStartPos.character // Correct selection range (start point)
        );
        locationLinks.push(node_1.LocationLink.create(targetUri, targetRange, targetSelectionRange, originSelectionRange));
    }
    connection.console.log(`[onDefinition] Returning ${locationLinks.length} mapped LocationLinks.`);
    return locationLinks;
});
// --- Diagnostics ---
function updateDiagnostics(htmlUriString) {
    ...
    const virtualPath = vscode_uri_1.URI.parse(virtualUriToUse).fsPath; // Need fsPath for LS call
    ...
    for (const virtualDiag of allVirtualDiagnostics) {
        if (virtualDiag.start === undefined || virtualDiag.length === undefined)
            continue;
        ...
        for (const mapping of docInfo.mappings) {
            ...
            if (Math.max(virtualDiagStart, mapping.virtualValueRange.start) < Math.min(virtualDiagEnd, mapping.virtualValueRange.end)) {
                ...
                const htmlDiagEndOffset = Math.min(htmlDiagStartOffset + virtualLengthInValue, // Use mapped length
                ...
                const htmlRange = node_1.Range.create(document.positionAt(htmlDiagStartOffset), document.positionAt(htmlDiagEndOffset));
...
function mapDiagnosticSeverity(category) {
    switch (category) {
        ...
        default: return node_1.DiagnosticSeverity.Information;
    }
}
// Helper to decode TS Semantic Classification
function decodeToken(classification) {
    ...
    if (classification > semanticClassificationFormatShift) {
        ...
        switch (type) {
            ...
            case classificationTypeMethodName: // 11
                typeIndex = legend.tokenTypes.indexOf(node_1.SemanticTokenTypes.method);
                break;
            case classificationTypeFunctionName: // 10
                typeIndex = legend.tokenTypes.indexOf(node_1.SemanticTokenTypes.function);
                break;
            case classificationTypePropertyName: // 9
                typeIndex = legend.tokenTypes.indexOf(node_1.SemanticTokenTypes.property);
                break;
            case classificationTypeVariableName: // 7
                typeIndex = legend.tokenTypes.indexOf(node_1.SemanticTokenTypes.variable);
                break;
            ...
            case ts.ClassificationType.className:
                ...
                if (type === 11) {
                    ...
                    typeIndex = legend.tokenTypes.indexOf(node_1.SemanticTokenTypes.class);
                }
                break;
            case ts.ClassificationType.enumName:
                typeIndex = legend.tokenTypes.indexOf(node_1.SemanticTokenTypes.enumMember);
                break;
            case ts.ClassificationType.interfaceName:
                typeIndex = legend.tokenTypes.indexOf(node_1.SemanticTokenTypes.interface);
                break;
            case ts.ClassificationType.moduleName:
                typeIndex = legend.tokenTypes.indexOf(node_1.SemanticTokenTypes.namespace);
                break;
            case ts.ClassificationType.typeAliasName:
                typeIndex = legend.tokenTypes.indexOf(node_1.SemanticTokenTypes.type);
                break;
            case ts.ClassificationType.typeParameterName:
                typeIndex = legend.tokenTypes.indexOf(node_1.SemanticTokenTypes.typeParameter);
                break;
            case ts.ClassificationType.parameterName:
                typeIndex = legend.tokenTypes.indexOf(node_1.SemanticTokenTypes.parameter);
                break;
            case ts.ClassificationType.keyword:
                typeIndex = legend.tokenTypes.indexOf(node_1.SemanticTokenTypes.keyword);
                break;
        ...
        if (modifier & tokenClassDeclarationMask) {
            const modifierIndex = legend.tokenModifiers.indexOf(node_1.SemanticTokenModifiers.declaration);
            if (modifierIndex !== -1)
                modifierSet |= (1 << modifierIndex);
        }
        if (modifier & tokenClassReadonlyMask) {
            const modifierIndex = legend.tokenModifiers.indexOf(node_1.SemanticTokenModifiers.readonly);
            if (modifierIndex !== -1)
                modifierSet |= (1 << modifierIndex);
        }
        // --- End Modifier Logic ---
...
connection.languages.semanticTokens.on(async (params) => {
    ...
    try {
        ...
        for (let i = 0; i < classifications.spans.length; i += 3) {
            ...
            // +++++++++++++++++++++++++++++++
            if (!decoded)
                continue; // Skip if we couldn't map the token type/modifier
...
connection.onCodeAction(async (params) => {
    ...
    let virtualCodeActionEndOffset;
    if (primaryMapping.wasThisPrepended) {
        const baseVirtualOffset = primaryMapping.virtualValueRange.start + 6;
    ...
    try {
        ...
        const codeFixes = languageService.getCodeFixesAtPosition(virtualFsPath, // <<< Use fsPath
        virtualCodeActionOffset, virtualCodeActionEndOffset, errorCodes, // Provide specific error codes from context if possible
        {}, // formatOptions
        {} // preferences
        );
        ...
        for (const fix of codeFixes) {
            ...
            if (fix.changes.length === 1 && fix.changes[0].fileName === docInfo.virtualUri) {
                ...
                for (const textChange of virtualChange.textChanges) {
                    ...
                    let editMapping = docInfo.mappings.find(m => m.virtualValueRange.start <= virtualStart && virtualEnd <= m.virtualValueRange.end);
                    // Fallback to primary mapping if no single mapping contains the edit
                    if (!editMapping) {
                        editMapping = primaryMapping;
                    }
                    if (!editMapping)
                        continue; // Skip if still no relevant mapping
                    ...
                    const htmlRange = node_1.Range.create(document.positionAt(htmlStartOffset), document.positionAt(htmlEndOffset));
                    // Use the *original* newText from the TS fix
                    mappedEdits.push(node_1.TextEdit.replace(htmlRange, textChange.newText));
                }
                if (mappedEdits.length > 0) {
                    ...
                    const codeAction = node_1.CodeAction.create(fix.description, workspaceEdit, node_1.CodeActionKind.QuickFix // Or map from fix.fixKind?
                    );
...
connection.onRenameRequest(async (params) => {
    ...
    const renameLocations = languageService.findRenameLocations(virtualFsPath, // <<< Use fsPath
    virtualOffset, false, // findInStrings
    ...
    for (const location of renameLocations) {
        ...
        // ++++++++++++++++++++++++++++++++
        let targetUri = vscode_uri_1.URI.file(targetFsPathRaw).toString(); // Use original raw path for URI creation
        ...
        const normalizedVirtualFsPath = vscode_uri_1.URI.parse(docInfo.virtualUri).fsPath.replace(/\\/g, '/');
        if (targetFsPath === normalizedVmFsPath) {
            ...
            if (vmSourceFile) {
                const vmDocument = vscode_languageserver_textdocument_1.TextDocument.create(targetUri, 'typescript', 0, ts.sys.readFile(targetFsPathRaw) ?? ''); // Create temporary doc for offset mapping
                if (vmDocument) {
                    targetRange = node_1.Range.create(vmDocument.positionAt(virtualStart), vmDocument.positionAt(virtualEnd));
                }
            }
        }
        ...
        // +++ Normalize comparison paths +++
            ...
            let locationMapping = docInfo.mappings.find(m => Math.max(virtualStart, m.virtualValueRange.start) < Math.min(virtualEnd, m.virtualValueRange.end));
            if (!locationMapping) {
            ...
            targetRange = node_1.Range.create(document.positionAt(htmlStartOffset), document.positionAt(htmlEndOffset));
        ...
        // Case 3: Location is in some other file (e.g., imported utility type) - handle as is
            ...
            if (otherSourceFile) {
                ...
                targetRange = node_1.Range.create(startPos.line, startPos.character, endPos.line, endPos.character);
            }
        }
        ...
        if (targetRange) {
            ...
            editsByUri.get(targetUri)?.push(node_1.TextEdit.replace(targetRange, newName));
        }
    }
    ...
    for (const [uri, edits] of editsByUri.entries()) {
        ...
        let version = null; // Use null for unknown version
        const doc = documents.get(uri);
        if (doc) {
            version = doc.version;
        }
        // If version is null, the client might ignore the version check.
        documentChanges.push(node_1.TextDocumentEdit.create({ uri, version }, edits));
    }
...
connection.onReferences(async (params) => {
    ...
    const referencedSymbols = languageService.findReferences(virtualFsPath, // <<< Use fsPath
    virtualOffset);
    ...
    for (const symbol of referencedSymbols) {
        ...
        for (const reference of symbol.references) {
            ...
            const targetUri = vscode_uri_1.URI.file(targetFsPath).toString();
            ...
            if (targetFsPath === docInfo.vmFsPath) {
                const vmDocument = vscode_languageserver_textdocument_1.TextDocument.create(targetUri, 'typescript', 0, ts.sys.readFile(targetFsPath) ?? '');
                if (vmDocument) {
                    targetRange = node_1.Range.create(vmDocument.positionAt(virtualStart), vmDocument.positionAt(virtualEnd));
                }
            }
            // Case 2: Location is in the Virtual File (needs mapping back to HTML)
            else if (targetFsPath === vscode_uri_1.URI.parse(docInfo.virtualUri).fsPath) {
                let locationMapping = docInfo.mappings.find(m => m.virtualValueRange.start <= virtualStart && virtualEnd <= m.virtualValueRange.end);
                if (!locationMapping) {
            ...
            // Case 3: Location is in some other TS file
                ...
                const otherDocument = vscode_languageserver_textdocument_1.TextDocument.create(targetUri, 'typescript', 0, ts.sys.readFile(targetFsPath) ?? '');
                if (otherDocument) {
                    targetRange = node_1.Range.create(otherDocument.positionAt(virtualStart), otherDocument.positionAt(virtualEnd));
                }
            }
            // Add the mapped location if range was determined
            if (targetRange) {
                locations.push(node_1.Location.create(targetUri, targetRange));
            }
        }
    }
...
connection.onHover(async (params) => {
    ...
    if (activeMapping) {
        ...
        const originVirtualLength = originVirtualSpan.length;
        const originVirtualEnd = originVirtualStart + originVirtualLength;
        const mapOffset = (virtualOffset, mapping) => {
            const valueVirtualStart = mapping.virtualValueRange.start;
            ...
            if (mapping.wasThisPrepended) {
                if (clampedVirtualOffset < valueVirtualStart + thisPrefixLength)
                    return null;
                else
                    return htmlExprStart + (clampedVirtualOffset - (valueVirtualStart + thisPrefixLength));
            }
            else {
        ...
        if (htmlStartOffsetNullable !== null && htmlEndOffsetNullable !== null) {
            ...
            if (htmlStartOffset < htmlEndOffset) { // Only create range if valid
                htmlRange = node_1.Range.create(document.positionAt(htmlStartOffset), document.positionAt(htmlEndOffset));
            }
        }
        else {
...
function getBindablePropertiesFromClassNode(classNode, sourceFile) {
    const bindables = [];
