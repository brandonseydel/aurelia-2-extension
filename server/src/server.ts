...
import { createConnection, ProposedFeatures, TextDocuments, InitializeParams, InitializeResult, TextDocumentSyncKind, CompletionItem, CompletionParams, Position, CompletionItemKind, DefinitionParams, LocationLink, Range as LSPRange } from 'vscode-languageserver/node';
import { TextDocument } from 'vscode-languageserver-textdocument';
import * as path from 'path';
import { URI } from 'vscode-uri';
import * as parse5 from 'parse5';
import { Document, Element, Node, TextNode } from 'parse5/dist/tree-adapters/default';

const connection = createConnection(ProposedFeatures.all);
const documents = new TextDocuments(TextDocument);

// Type for storing detailed mapping info
interface DetailedMapping {
    htmlStart: number; htmlEnd: number; 
    blockStart: number; blockEnd: number; // Range of the entire { ... } block in virtual file
    valueStart: number; valueEnd: number; // Range of the expression value inside the block
    wasThisPrepended: boolean; // If ANY \'this.\' was prepended to a member
    endsWithPendingTerm: boolean; // <<< ADD BACK: If a trailing \'_this.\' was added
}

// Add a new interface to hold both mappings and member names
interface MappingInfo {
    mappings: DetailedMapping[];
    memberNames: string[]; // Store member names used for this virtual file
}

let languageService: ts.LanguageService;
let virtualFiles: Map<string, { content: string; version: number }> = new Map();
let mappings: Map<string, MappingInfo> = new Map();
...
function needsThisPrefix(expression: string): boolean {
...
function extractExpressions(htmlContent: string): { expression: string; start: number; end: number }[] {
...
function getViewModelMemberNames(vmClassName: string, vmFsPath: string): string[] {
    ...
    try {
        ...
        if (program) {
            ...
            } else { connection.console.warn(`[getViewModelMemberNames] Could not get source file object for ${vmFsPath}.`); }
        } else { connection.console.warn('[getViewModelMemberNames] Could not get program from language service.'); }
    } catch (e) {
...
function updateVirtualFile(htmlUri: string, htmlContent: string) {
  ...
  const virtualFsPath = URI.parse(virtualFileUri).fsPath;
  ...
  for (const expr of expressions) {
    ...
    let transformedExpression: string;
...
function mapCompletionKind(kind: string): CompletionItemKind | undefined {
  const kindMap = {
    ...
    [ts.ScriptElementKind.keyword]: CompletionItemKind.Keyword, // Added keyword
  };
  return kindMap[kind as keyof typeof kindMap];
}
...
function createLanguageServiceInstance(documents: TextDocuments<TextDocument>, virtualFiles: Map<string, { content: string; version: number }>, workspaceRoot: string): ts.LanguageService {
  ...
  if (configFileName) {
    ...
    if (configFile.error) {
      ...
      const parsedConfig = ts.parseJsonConfigFileContent(
        ...
        { // Provide initial default values that might be overridden
          strict: strictMode, 
  ...
  const allTsFiles = new Set<string>();
  
  // Collect all .ts files to help with module resolution
  function scanTsFiles(directory: string, result: Set<string>) {
    try {
      ...
      for (const entry of entries) {
        ...
        if (isDirectory) {
          ...
          result.add(URI.file(fullPath).toString());
        }
      }
    } catch (e) {
  ...
  const host: ts.LanguageServiceHost = {
    ...
    getScriptSnapshot: (fileName) => {
      ...
      try {
        if (fileName.startsWith('file:///')) {
            fsPath = URI.parse(fileName).fsPath;
        }
    ...
    resolveModuleNames: (moduleNames, containingFile, _reusedNames, _redirectedReference, options) => {
      ...
      return moduleNames.map(moduleName => {
        ...
        try {
            containingDir = path.dirname(containingFile.startsWith('file:///') ? URI.parse(containingFile).fsPath : containingFile);
        } catch (e) {
...
connection.onCompletion((params: CompletionParams): CompletionItem[] => {
  ...
  virtualCompletionOffset = Math.max(
      activeMapping.valueStart, 
      Math.min(virtualCompletionOffset, activeMapping.valueEnd)
  ); 
  ...
  let result: CompletionItem[] = completions.entries
    .filter(entry => {
      // Always filter internal helpers
      if (entry.name.startsWith('___')) { return false; } 
      ...
      return desiredKinds.includes(entry.kind as ts.ScriptElementKind);
    })
    .map(entry => {
        // --- REVISED Sort Priority --- 
        let sortPriority = '9'; // Default lowest priority
        const kind = entry.kind as ts.ScriptElementKind;
        ...
        const resultItem: CompletionItem = {
            label: entry.name,
  ...
  if (needsThisFallback && !result.some(item => item.data?.isViewModelMember)) {
    ...
    const fallbackCompletions = languageService.getCompletionsAtPosition(
      virtualFileUri, 
      activeMapping.valueStart, // Get completions at the start of the value expression (context of 'this')
      {}
    );
    ...
    if (fallbackCompletions) {
      ...
      const fallbackResult = fallbackCompletions.entries
        ...
        .map(entry => {
          ...
          const fallbackItem: CompletionItem = {
...
connection.onDefinition(async (params: DefinitionParams): Promise<LocationLink[] | undefined> => {
  ...
  const uri = params.textDocument.uri;
  if (!uri.endsWith('.html')) return undefined;
  ...
  if (originalHtmlExpression.trim() === '') {
    ...
    virtualOffset = Math.max(
      activeMapping.valueStart,
      Math.min(activeMapping.valueStart + relativeHtmlOffsetForCursor, activeMapping.valueEnd)
    );
  ...
  let relativeHtmlStartOffset: number;
  ...
  const originSelectionRange = LSPRange.create(
      document.positionAt(originHtmlStart),
      document.positionAt(originHtmlEnd)
  );
  ...
  for (const def of definitions.definitions) {
    ...
    const targetUri = def.fileName.startsWith('file:///') 
        ? def.fileName 
        : URI.file(def.fileName).toString(); 
    ...
    const targetRange = LSPRange.create(
      ...
      targetEndPos.line, targetEndPos.character
    );
    ...
    locationLinks.push(
      LocationLink.create(targetUri, targetRange, targetSelectionRange, originSelectionRange) // Use specific selection range
    );
  }
